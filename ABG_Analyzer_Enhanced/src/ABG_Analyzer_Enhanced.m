classdef ABG_Analyzer_Enhanced < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function ApplyCorrectionButtonPushed(app, ~)
                        ...
                        for i = 1:n
                            ...
                            try
                                ...
                                switch ci
                                    ...
                                    case 6 
                                        lv=best_lam_h;
                                        yFastTr=cdi_fast_paired_fit(leaveIdx);
                                        yFastTe=cdi_fast_paired_fit(i);
                                        cMaskH=robustCleanMask(app,xTr,yFastTr);
                                        xCleanH=xTr(cMaskH); yCleanH=yFastTr(cMaskH); nT=numel(xCleanH);
                                        if nT>=2
                                            xmT=mean(xCleanH,'omitnan'); ymT=mean(yCleanH,'omitnan');
                                            sxxT=sum((xCleanH-xmT).^2,'omitnan')/(nT-1);
                                            syyT=sum((yCleanH-ymT).^2,'omitnan')/(nT-1);
                                            sxyT=sum((xCleanH-xmT).*(yCleanH-ymT),'omitnan')/(nT-1);
                                            denom=2*sxyT;
                                            if abs(denom)>1e-10
                                                slopeT=(syyT-lv*sxxT+sqrt((syyT-lv*sxxT)^2+4*lv*sxyT^2))/denom;
                                                intT=ymT-slopeT*xmT;
                                                if abs(slopeT)>1e-5, pred=(yFastTe-intT)/slopeT;
                                                else, pred=yFastTe-(ymT-xmT); end
                                            else, pred=yFastTe-(ymT-xmT); end
                                        else, pred=NaN; end
                                end
            ...
            elseif biasImproved && sdImproved
                verdict = '✓ GOOD';  vColor = [0.1 0.6 0.1];
            elseif biasImproved
        ...
        function ComparePlotsButtonPushed(app, ~)
            ...
            uimenu(mFile, 'Text', 'Save Figure As...   (Ctrl+S)', 'Accelerator', 'S', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.saveCompareFigureAs(fig, param));
            uimenu(mFile, 'Text', 'Copy Figure to Clipboard', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.copyCompareFigure(fig));
            uimenu(mFile, 'Text', 'Print...', 'Separator', 'on', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.printCompareFigure(fig));
            ...
            uimenu(mExport, 'Text', 'Save as SVG (vector)...', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'svg'));
            uimenu(mExport, 'Text', 'Save as PNG (high resolution)...', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'png'));
            uimenu(mExport, 'Text', 'Save as PDF...', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'pdf'));
            uimenu(mExport, 'Text', 'Save as JPEG...', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'jpg'));
            uimenu(mExport, 'Text', 'Save as TIFF (300 DPI)...', 'Separator', 'on', ...
                'MenuSelectedFcn', @(~,~) ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'tif'));
        ...
        function ShowFormulaButtonPushed(app, ~)
            ...
            switch mdl.type
                case 'hybrid'
                    ...
                    latexStr{end+1} = sprintf('$$ \\τ = %.1f \\text{ min}, \\quad \\beta_0 = %.4f, \\quad \\beta_1 = %.4f $$', mdl.tau, mdl.intercept, mdl.slope);
                case 'bias'
                    yCorr = yC - mdl.bias;
            ...
            expStatsBtn.ButtonPushedFcn = @(~,~) ABG_Analyzer_Enhanced.exportStatsAsSVG(statsTextLines, param, mName);
            ...
            if biasAfter < 0.5 && sdAfter < 2
                interpLines{end+1} = 'Correction quality: GOOD  -  residual bias near zero, tight agreement.';
            elseif biasAfter < 1.0
                interpLines{end+1} = 'Correction quality: MODERATE  -  bias reduced but some scatter remains.';
            else
                interpLines{end+1} = 'Correction quality: LIMITED  -  consider if systematic drift or outliers are present.';
            end
        ...
        function createComponents(app)
            ...
            app.UIFigure.Name = 'ABG Analyzer - State-Locked Edition v9.9';
    ...
    methods (Static)
        ...
        function saveCompareFigureAs(fig, param)
            ...
            switch choice
                case 'SVG (vector)'
                    ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'svg');
                case 'PNG (300 DPI)'
                    ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'png');
                case 'PDF'
                    ABG_Analyzer_Enhanced.exportCompareFigure(fig, param, 'pdf');
            end
        end
    ...
    methods (Access = public)
        function app = ABG_Analyzer_Enhanced
